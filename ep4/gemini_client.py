"""
Reusable Gemini Client — Episode 04 (RAG)
==========================================
Multi-provider fallback: Gemini → Groq → Cerebras
Includes: generate(), embed(), embed_batch(), count_tokens()
"""
...
def embed(text: str, model: str = EMBEDDING_MODEL) -> list[float]:
    """Embed a single text string (Gemini only)."""
    client = get_client()
    for attempt in range(MAX_RETRIES):
        try:
...
def embed_batch(texts: list[str], model: str = EMBEDDING_MODEL) -> list[list[float]]:
    """Embed multiple texts in a single API call (Gemini only)."""
    client = get_client()
    for attempt in range(MAX_RETRIES):
        try:
...
if __name__ == "__main__":
    ...
    print(f"Embed dimension: {len(vec)}")
